...
public class MoonshinUI : MonoBehaviour
{
    ...
    {
        // For Level 1 Items
        foreach (CraftingItem craftingItem in moonshine.craftingItemsLevel1)
        {
            AutoAssignCraftingItemProperties(craftingItem);
        }

        // For Level 2 Items
        foreach (CraftingItem craftingItem in moonshine.craftingItemsLevel2)
        {
            AutoAssignCraftingItemProperties(craftingItem);
        }
    }
   ...
   public void DisplayCraftingItems()
    {
        ...
        // Get the moonshine's current level
        int moonshineLevel = moonshine.upgradeBuilding.currentLevel;
        ...
        availableCraftingItems.AddRange(moonshine.craftingItemsLevel1);

        // If level >= 2, include level 2 items
        if (moonshineLevel >= 2)
        {
            availableCraftingItems.AddRange(moonshine.craftingItemsLevel2);
        }
        ...
        foreach (CraftingItem craftingItem in orderedCraftingItems)
        {
            ...
            MoonshineItemUI moonshineItemUI = newCraftingItemUI.GetComponent<MoonshineItemUI>();
            if (moonshineItemUI  != null)
            {
                moonshineItemUI.Initialize(craftingItem, this);
            }
        }
    }
    ...
    public void DisplayActiveCraftingJobs()
    {
        ...
        foreach (CraftingJob job in craftManager.MoonshienactiveJobs)
        {
